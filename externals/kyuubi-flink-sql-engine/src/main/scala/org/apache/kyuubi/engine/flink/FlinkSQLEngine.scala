...
import org.apache.kyuubi.config.KyuubiConf.{ENGINE_FLINK_INITIALIZE_SQL, ENGINE_INITIALIZE_SQL}
...
object FlinkSQLEngine extends Logging {
  ...
  private def bootstrap(executionTarget: String) = {
    ...
    val initializeSQLs =
      kyuubiConf.get(ENGINE_INITIALIZE_SQL) ++ kyuubiConf.get(ENGINE_FLINK_INITIALIZE_SQL)
    initializeSQLs.foreach { stmt =>
